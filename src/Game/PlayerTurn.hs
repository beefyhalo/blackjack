...
decideDoubleDown pid = \case
  Game {state = OpeningTurnState ctx}
    | Set.member pid ctx.readyPlayers -> Left PlayerAlreadyActed
    | otherwise ->
        withPlayerRound pid ctx.insuranceContext.context.rounds \playerRound ->
          let currentHand = Z.current playerRound.hands
           in withValidBet (currentHand.bet * 2) playerRound.player.stack.chips \_ ->
...
decideSurrender pid = \case
  Game {state = OpeningTurnState ctx}
    | Set.member pid ctx.readyPlayers -> Left PlayerAlreadyActed
    | otherwise -> withPlayerRound pid ctx.insuranceContext.context.rounds \_ -> Right (PlayerSurrendered pid)
  _ -> Left BadCommand
...
decideSplit pid = \case
  Game {state = OpeningTurnState ctx}
    | Set.member pid ctx.readyPlayers -> Left PlayerAlreadyActed
    | otherwise -> withPlayerRound pid ctx.insuranceContext.context.rounds \round ->
...
evolveOpeningTurn game@Game {state = OpeningTurnState ctx} event =
  case event of
    PlayerSplitHand pid c1 c2 d1 d2 ->
      EvolutionResult game {state = OpeningTurnState ctx {insuranceContext, readyPlayers}}
      where
        deck' = deck {drawn = deck.drawn + 2}
  ...
  where
    ...
    splitPlayerHand card1 card2 draw1 draw2 round = round {hands}
      where
        hands =
      ...
      let intermediate = evolvePlayerTurn game {state = PlayerTurnState ctx.insuranceContext} event
       in case intermediate of
            ...
            EvolutionResult next@Game {state = PlayerTurnState insuranceContext}
              ...
              | otherwise -> EvolutionResult next {state = OpeningTurnState OpeningContext {insuranceContext, readyPlayers}}
              where
                round = insuranceContext.context.rounds Map.! pid
...
evolvePlayerTurn game@Game {state = PlayerTurnState insuranceContext} = \case
  HitCard pid card -> advanceState nextDeck pid (modifyCurrentHand \h -> h {hand = addCard card h.hand})
  PlayerStood pid -> advanceState deck pid (modifyCurrentHand \h -> h {hasStood = True})
  PlayerDoubledDown pid card -> advanceState nextDeck pid $
    modifyCurrentHand \h -> h {hand = addCard card h.hand, bet = h.bet * 2, hasDoubledDown = True}
  PlayerSurrendered pid -> advanceState deck pid (\round -> round {hasSurrendered = True})
  _ -> EvolutionResult game
  where
    ...
    advanceState deck' pid adjustRound
      ...
      where
        moveHandFocus round
          | hasCompletedTurn round = round {hands = fromMaybe (Z.start round.hands) (Z.right round.hands)}
          | otherwise = round
