...
decideDealing Game {state = DealingState pids deck} = \case
  DealInitialCards -> maybe (Left EmptyDeck) Right do
...
evolveDealing game@Game {state = DealingState players deck} = \case
  CardsDealt playerHands dealer
    | isAce (visibleCard dealer) -> EvolutionResult game {state = OfferingInsuranceState context}
    | otherwise -> EvolutionResult game {state = OpeningTurnState openingContext}
    where
      openingContext = OpeningContext (InsuranceContext context Map.empty) Set.empty
      context = GameContext deck' rounds' dealer
        where
          deck' =
            let cardsDrawn = sum [handSize hand | (_, hand) <- playerHands] + handSize dealer.dealerHand
             in deck {drawn = deck.drawn + cardsDrawn}
          rounds' =
            Map.fromList [(pid, initPlayerRound hand (players Map.! pid)) | (pid, hand) <- playerHands]